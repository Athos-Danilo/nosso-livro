...
- [ ] **M1.3: Migrations de Banco**
  - [ ] Executar `npx prisma migrate dev --name cria_tabela_notificacoes`, garantindo a criação de índices de busca para `Notificacao.idUsuario` e `Notificacao.estado`.
...
- [ ] **M2.1: Desenvolvimento do Cliente de Usuários**
  - [ ] Criar o `ClienteUsuario` utilizando `axios` para efetuar chamadas a `GET /api/usuarios/{id}` do **Serviço de Autenticação e Usuário**.
  - [ ] Resgatar nome, e-mail, whatsapp e status do usuário a ser notificado.
- [ ] **M2.2: Resiliência de Rede**
  - [ ] Configurar timeout baixo (máximo 3 segundos) para evitar travamento do consumidor de filas.
  - [ ] Tratar cenários em que o usuário não é encontrado (`404`) ou o serviço está indisponível (`503`) de forma controlada.
...
- [ ] **M3.1: Conexão e Auto-reconexão com o Broker**
  - [ ] Desenvolver classe utilitária de conexão em `/src/fila/rabbitmq.ts` utilizando a biblioteca `amqplib` com auto-reconexão.
- [ ] **M3.2: Configuração de Filas e Consumidores**
  - [ ] Declarar exchange do tipo `topic` e criar a fila exclusiva `fila_notificacoes`.
...
- [ ] **M4.1: Configuração do Nodemailer**
  - [ ] Desenvolver o módulo de envio em `/src/servicos/email.servico.ts`.
  - [ ] Configurar o transporte SMTP utilizando credenciais seguras carregadas de variáveis de ambiente (`SMTP_HOST`, `SMTP_PORT`, `SMTP_USUARIO`, `SMTP_SENHA`).
- [ ] **M4.2: Construção dos Templates de E-mail (HTML)**
  - [ ] Desenvolver templates de e-mail HTML modernos e responsivos em português:
...
- [ ] **M4.3: Mock de Disparo de WhatsApp**
  - [ ] Criar classe mockada que simula o envio de alertas por WhatsApp (gravando os logs estruturados no console para fins de demonstração).
...
- [ ] **M5.1: Fluxo de Processamento de Notificação**
  ...
  - [ ] Chamar `ClienteUsuario` via HTTP para buscar dados de e-mail e nome do usuário.
  - [ ] Renderizar o template de e-mail correto com as variáveis reais do payload.
  - [ ] Enviar o e-mail via Nodemailer.
  - [ ] Registrar o status do disparo (`ENVIADO` ou `FALHOU` com a mensagem do erro) na tabela `Notificacao` do banco de dados `banco_notificacoes`.
- [ ] **M5.2: Política de Retentativas e Tratamento de Falhas**
  - [ ] Se o disparo SMTP falhar por instabilidade no servidor de e-mails, salvar a notificação com estado `FALHOU` e aplicar lógica de reprocessamento automático (ex: 3 tentativas de envio antes de desistir).
...
- [ ] **M6.1: Testes de Integração com Ethereal Email**
  - [ ] Configurar nos testes o envio de e-mails reais através de contas temporárias criadas dinamicamente no serviço **Ethereal Email**. Validar a entrega e o visual do HTML.
- [ ] **M6.2: Testes de Integração de Mensageria**
  - [ ] Criar testes que enviam eventos artificiais no RabbitMQ e validam se o consumidor aciona corretamente o motor de e-mail e grava o histórico correspondente no banco.
...
- [ ] **M7.1: Dockerfile Multi-stage Otimizado**
  - [ ] Compilar TypeScript para JavaScript e gerar imagem final baseada em `node:18-alpine`.
- [ ] **M7.2: Desligamento Gracioso (Graceful Shutdown)**
  - [ ] Interceptar sinais `SIGINT`/`SIGTERM`: parar o servidor Express de health check, fechar consumo do RabbitMQ e fechar as conexões do Prisma Client de forma limpa.
- [ ] **M7.3: Health Check e Logs Estruturados**
  - [ ] Criar rotas `/saude` e `/pronto`.
  - [ ] Logs estruturados JSON em português utilizando Winston ou Pino para registrar o status dos disparos.
